...
<xsl:for-each select ="/computer/storage/drives/drive" >
	<tr>
		<td><strong><xsl:value-of select = "@Mounted"/></strong></td>
		<td><xsl:value-of select = "@Filesystem"/></td>
		<td><xsl:value-of select = "@Size"/></td>
		<td><xsl:value-of select = "@Avail"/></td>
		<td><xsl:value-of select = "@Used"/><xsl:value-of select = "@UsePer"/>)</td>
	</tr>
</xsl:for-each> 
</table>
...
<xsl:if test="computer/Sockets">
<h2 id="miscellaneous_virtualmemory">Sockets Opened</h2>
<table>
	<tr>
		<th>Protocal</th>
		<th>Local Address</th>
	...
	<xsl:for-each select ="/computer/Sockets/Socket">
		<tr>
			<td><xsl:value-of select = "@protocal"/></td>
			<td><xsl:value-of select = "@localAddress"/></td>
			<td><xsl:value-of select = "@foreignAddress"/></td>
			<td><xsl:value-of select = "@pid_programName"/></td>
		</tr>
	</xsl:for-each> 
</table>
</xsl:if>
...
<xsl:if test="computer/memory/swap">
<h2 id="miscellaneous_virtualmemory">Swap Memory</h2>
<table>
	<tr>
		<th>Filename</th>
		<th>Type</th>
	...
	<xsl:for-each select ="/computer/memory/swap">
		<tr>
